...
public class EventPanel extends JPanel
{
	 ...
	 */
	public EventPanel(TreeSet<Event> events, int boxWidth, int hourBoxHeight)
	{
		BOX_WIDTH = boxWidth;
		...
		this.setPreferredSize(new Dimension(BOX_WIDTH + 2, HOUR_BOX_HEIGHT * 24 + 1));
	...
	private void createBackground(Graphics2D g2)
	{
		...
		for (int i = 0; i < 24; i++)
		{
			Rectangle r = new Rectangle(0, i * HOUR_BOX_HEIGHT, BOX_WIDTH + 1, HOUR_BOX_HEIGHT);			
	...
	private void createEvents(Graphics2D g2)
	{
		...
		for (Event e : events)
		{
			...
			Rectangle r = new Rectangle(1, startCal.get(Calendar.HOUR_OF_DAY) * HOUR_BOX_HEIGHT + startCal.get(Calendar.MINUTE) / 60 * HOUR_BOX_HEIGHT, BOX_WIDTH, (int) (minutes / 60 * HOUR_BOX_HEIGHT));
			eventRectangles.add(r);
			
			
			String name = e.getName();
			if (eventDetails.containsKey(name))
				eventDetails.get(name).add(new Point(3, 12 + startCal.get(Calendar.HOUR_OF_DAY) * HOUR_BOX_HEIGHT + startCal.get(Calendar.MINUTE) / 60 * HOUR_BOX_HEIGHT));
			else
			{
			...
			if (eventDetails.containsKey(times))
				eventDetails.get(times).add(new Point(3, 24 + startCal.get(Calendar.HOUR_OF_DAY) * HOUR_BOX_HEIGHT + startCal.get(Calendar.MINUTE) / 60 * HOUR_BOX_HEIGHT));
			else
			{
